...
export default function SuperAdminPage() {
  ...
  const loadData = async () => {
    ...
    console.log('invoice owner_ids:', (invoicesData || []).map(i => i.owner_id))
    ...
    <div className="min-h-screen overflow-x-hidden bg-gray-50 flex" style={{ fontFamily: "'DM Sans', system-ui, sans-serif" }}>
                ...
                <div className={`ml-3 pl-3 mb-1 border-l ${item.key === 'reklamcilar' ? 'border-amber-800' : 'border-gray-700'}`}>
                  {item.children.map(child => (
                    <button key={child.key} onClick={() => { setActiveTab(child.key); setMobileMenuOpen(false) }}
                      className={`w-full text-left px-3 py-1.5 rounded-lg text-sm transition-all mb-0.5 ${activeTab === child.key
      ...
      <div className={`${sidebarCollapsed ? 'ml-16' : 'ml-60'} flex-1 transition-all duration-200 min-w-0 overflow-x-hidden`}>
        ...
        <header className="sticky top-0 z-10 flex min-h-14 flex-wrap items-center gap-2 border-b border-gray-100 bg-white/80 px-4 py-2 shadow-sm backdrop-blur-sm sm:px-6">
          ...
          <div className="ml-auto flex w-full justify-end sm:w-auto" ref={profileMenuRef}>
            <div className="relative">
                  ...
                  <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
                    <div className="px-5 py-4 border-b border-gray-100">
                      ...
                      <div className="flex items-center justify-between mt-2">
                        <p className="text-xs text-gray-400">{filteredGodLeads.length} lead gösteriliyor</p>
                        <button onClick={async () => {
                          const XLSX = await import('xlsx')
                          const rows = filteredGodLeads.map(l => {
                            const owner = customers.find(c => c.id === l.owner_id)
                            return {
                              'Firma': owner?.company_name || owner?.full_name || '-',
                              'Ad Soyad': l.full_name || '',
                              'Telefon': l.phone || '',
                              'E-posta': l.email || '',
                              'Durum': l.status || '',
                              'Kaynak': l.source || '',
                              'Tutar': l.procedure_amount || 0,
                              'Tarih': new Date(l.created_at).toLocaleDateString('tr-TR'),
                            }
                          })
                          const ws = XLSX.utils.json_to_sheet(rows)
                          ws['!cols'] = [20, 20, 15, 25, 12, 14, 12, 12].map(w => ({ wch: w }))
                          const wb = XLSX.utils.book_new()
                          XLSX.utils.book_append_sheet(wb, ws, 'Leadler')
                          XLSX.writeFile(wb, `DataPilot_GodMode_${new Date().toLocaleDateString('tr-TR').replace(/\./g, '-')}.xlsx`)
                        }}
