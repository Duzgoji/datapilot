...
export default function SuperAdminPage() {
  ...
  const loadData = async () => {
    ...
    const advertisersWithSubs = (advertisersData || []).map((a: any) => ({
      ...a,
      advertiser_subscriptions: advSubsData?.filter(s => s.advertiser_id === a.id) || []
    }))
    setAdvertisers(advertisersWithSubs)
  }
                      ...
                      <div key={c.id} className={`px-5 py-3.5 grid grid-cols-6 gap-2 items-center ${i < customers.length - 1 ? 'border-b border-gray-50' : ''} hover:bg-gray-50/50 transition-colors`}>
                        ...
                        <div>
                          <select
                            ...
                            className={`text-xs font-semibold px-2 py-1 rounded-lg border-0 cursor-pointer focus:outline-none focus:ring-2 focus:ring-indigo-500 ${sub?.plan === 'enterprise' ? 'bg-amber-50 text-amber-700' :
                        ...
                        <div className="text-right">
                          <button
                            ...
                            className={`text-xs font-medium px-2.5 py-1 rounded-full transition-colors ${c.is_active !== false
