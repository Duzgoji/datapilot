    ...
    <div className="min-h-screen bg-gray-100 flex items-center justify-center">
      <div className="text-center">
        <img src="/logo.png" alt="DataPilot" className="h-7 w-auto" />
                <p className="text-gray-500 text-sm">Y?kleniyor...</p>
      </div>
    </div>
  )
              ...
              <div className="grid grid-cols-1 gap-4 xl:grid-cols-3">
                    ...
                    <div key={lead.id} className="px-4 py-3 flex items-center justify-between border-b border-gray-50 last:border-0 hover:bg-gray-50">
                      <div>
                        <div className="flex items-center gap-2">
                          <p className="font-medium text-gray-900 text-sm">{lead.full_name || '?simsiz'}</p>
                ...
                <div className="space-y-4">
                  <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
                    ...
                    {branches.length === 0 && <p className="text-xs text-gray-400">Hen?z ?ube yok.</p>}
                  ...
                  <form onSubmit={handleAddLead} className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    ...
                    <div>
                      ...
                      <select value={leadSource} onChange={e => setLeadSource(e.target.value)}
                        className="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                        <option value="manual">Manuel Giri?</option>
                    ...
                    {inviteLink ? (
                      <div className="bg-green-50 border border-green-200 rounded-xl p-4">
                        ...
                        <div className="flex flex-col gap-2 sm:flex-row">
                          ...
                          <button onClick={() => { navigator.clipboard.writeText(inviteLink); alert('Kopyaland?!') }}
                      ...
                      <form onSubmit={handleAddBranch} className="grid grid-cols-2 gap-4">
                        <div>
                          <label className="block text-xs font-medium text-gray-600 mb-1">?ube Ad? *</label>
                          <input value={branchName} onChange={e => setBranchName(e.target.value)} required
                            className="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                            placeholder="?ube ad?" />
                        </div>
                        <div>
                          <label className="block text-xs font-medium text-gray-600 mb-1">?leti?im Ki?isi</label>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-100">
                  {branches.length === 0 ? (
                    <div className="p-10 text-center">
                      ...
                      <p className="text-gray-500 text-sm">Hen?z ?ube yok.</p>
                    </div>
                  ) : branches.map(b => (
