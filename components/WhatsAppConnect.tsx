...
export default function WhatsAppConnect({ ownerId }: { ownerId: string }) {
  ...
  const handleDisconnect = async () => {
    ...
    await supabase.from('whatsapp_connections').update({ is_active: false }).eq('owner_id', ownerId)
    await loadConnection()
  }
  ...
  return (
    <div className="max-w-2xl space-y-4">
      <div className="bg-white rounded-2xl border border-gray-100 p-6">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500">WhatsApp'tan gelen mesajları otomatik lead olarak kaydedin</p>
          </div>
            ...
            {!showForm ? (
              <div className="text-center py-4">
                ...
                <div className="bg-amber-50 border border-amber-100 rounded-xl p-4 mb-4 text-left">
                  <p className="text-amber-700 text-xs font-semibold mb-2">⚠️ Gereksinimler</p>
                  <ul className="text-amber-600 text-xs space-y-1">
                    <li>• Meta Business hesabı</li>
                    <li>• WhatsApp Business API erişimi (360dialog, WATI veya Meta Cloud API)</li>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input value={accessToken} onChange={e => setAccessToken(e.target.value)}
                    type="password" placeholder="WhatsApp Access Token"
                    className="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-green-500" />
                </div>
      ...
      <div className="bg-green-50 rounded-xl p-4 border border-green-100">
        <p className="text-green-700 text-xs font-medium mb-1">ℹ️ Nasıl çalışır?</p>
        <p className="text-green-600 text-xs">
          WhatsApp Business hesabınıza gelen her yeni mesaj otomatik olarak lead olarak kaydedilir.
          Aynı numaradan tekrar mesaj gelse de sadece bir lead oluşturulur.
        </p>
      </div>
    </div>
  )
