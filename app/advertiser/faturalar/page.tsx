...
export default function FaturalarPage() {
  ...
  const exportCSV = (type: 'invoices' | 'logs') => {
    const rows = type === 'invoices'
      ? [['M�steri', 'Tutar', 'Durum', 'Son �deme', 'Olusturulma'],
      ...
      : [['M�steri', 'Tutar', 'T�r', 'Not', 'Tarih'],
         ...filteredLogs.map(log => [getCustomerName(log.customer_id), log.amount, log.type === 'payment' ? '�deme' : log.type === 'refund' ? 'Iade' : 'D�zeltme', log.note || '-', new Date(log.created_at).toLocaleDateString('tr-TR')])]
  ...
  return (
    <div className="max-w-5xl space-y-5">
      ...
      {myInvoices.filter(i => i.status === 'pending').length > 0 && (
        <div className="flex flex-wrap items-center gap-3 rounded-2xl border border-amber-200 bg-amber-50 p-4">
          ...
          <p className="text-xs font-semibold text-amber-700">
            {myInvoices.filter(i => i.status === 'pending').length} bekleyen faturaniz var
          </p>
      ...
      <div className="flex w-full flex-wrap gap-1 rounded-xl bg-gray-100 p-1 sm:w-fit">
        {([['musteri', 'M�steri Faturalari'], ['bana-gelen', 'Bana Gelen Faturalar']] as const).map(([key, label]) => (
      ...
      {mainTab === 'musteri' && (
        <div className="space-y-5">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h2 className="text-base font-semibold text-gray-900">Faturalar & �deme Ge�misi</h2>
              <p className="text-xs text-gray-400 mt-0.5">T�m m�sterilerin fatura ve �deme loglari</p>
            </div>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
            {[
              ...
              { label: 'Gecikmis', value: `?${totalOverdue.toLocaleString()}`, sub: `${invoices.filter(i => i.status === 'overdue').length} fatura`, color: 'text-red-500' },
            ].map(k => (
          ...
          <div className="flex flex-wrap items-center gap-3 rounded-2xl border border-gray-100 bg-white px-4 py-4 sm:px-5">
            <div className="relative flex-1 min-w-[160px]">
              ...
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="M�steri ara..."
            ...
            <div className="relative w-full sm:w-auto">
              <select value={filterCustomer} onChange={e => setFilterCustomer(e.target.value)}
                ...
                <option value="all">T�m M�steriler</option>
          ...
          <div className="flex w-full flex-wrap gap-1 rounded-xl bg-gray-100 p-1 sm:w-fit">
            {([['faturalar', 'Faturalar'], ['log', '�deme Ge�misi']] as const).map(([key, label]) => (
              ...
              <div className="hidden md:block">
                <div className="px-5 py-3 bg-gray-50 border-b border-gray-100">
                  <div className="grid grid-cols-6 gap-2 text-xs font-semibold text-gray-400 uppercase tracking-wider">
                    <div className="col-span-2">M�steri</div>
                ...
                {filteredInvoices.length === 0 ? (
                  <div className="p-12 text-center"><p className="text-gray-400 text-sm">Fatura bulunamadi.</p></div>
                    ...
                    <div key={inv.id} className={`px-5 py-3.5 grid grid-cols-6 gap-2 items-center hover:bg-gray-50/50 transition-colors ${i < filteredInvoices.length - 1 ? 'border-b border-gray-50' : ''}`}>
                      ...
                      <div className="text-right flex items-center justify-end gap-1.5">
                        ...
                        {inv.status === 'pending' && isOverdue && (
                          <button onClick={() => markAsOverdue(inv)} className="text-xs text-red-500 font-medium px-2.5 py-1 hover:bg-red-50 rounded-lg transition-colors border border-red-200">Gecikmis</button>
                        )}
                      </div>
                    </div>
                  )
              ...
              <div className="hidden md:block">
                ...
                {filteredLogs.length === 0 ? (
                  <div className="p-12 text-center">
                    <p className="text-gray-400 text-sm">�deme kaydi yok.</p>
                    <p className="text-xs text-gray-300 mt-1">Faturalar "�dendi" isaretlenince burada g�r�n�r.</p>
                  </div>
                ) : filteredLogs.map((log, i) => {
                  const typeLabel = log.type === 'payment' ? '�deme' : log.type === 'refund' ? 'Iade' : 'D�zeltme'
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
            <div className="hidden md:block">
              <div className="px-5 py-3 bg-gray-50 border-b border-gray-100">
                <div className="grid grid-cols-5 gap-2 text-xs font-semibold text-gray-400 uppercase tracking-wider">
                  <div className="col-span-2">A�iklama</div>
                  <div className="text-right">Tutar</div>
                  <div className="text-center">Durum</div>
                  <div className="text-right">Islem</div>
                </div>
